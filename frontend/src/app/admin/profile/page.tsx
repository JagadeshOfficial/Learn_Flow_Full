...
import { Loader2, Camera, User, Mail, Shield, Smartphone, Calendar, Save, Settings, CheckCircle2, Lock } from "lucide-react";
...
export default function AdminProfilePage() {
    ...
    const fetchProfile = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/v1/auth/admin/me`, {
            ...
            if (res.ok) {
                ...
                localStorage.setItem('adminPhotoUrl', data.photoUrl || '');
            }
        } catch (err) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/v1/auth/admin/${profile.id}/avatar`, {
                method: 'POST',
                headers: {
            ...
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
                    <div className="space-y-2">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">Personal Profile</h1>
                        <p className="text-slate-500 text-lg font-medium">Update your professional information and account preferences.</p>
                    </div>
                    <Button
                        onClick={handleSave}
                            ...
                            <div className="flex flex-col items-center text-center space-y-6">
                                <div className="relative group/avatar">
                                    <Avatar className="h-40 w-40 border-8 border-slate-50 shadow-inner bg-slate-100 overflow-hidden transition-all duration-500 group-hover:border-slate-100">
                                        {profile.photoUrl ? (
                                            <AuthAvatar photoKey={profile.photoUrl} alt="Admin" size="lg" />
                                        ) : (
                        ...
                        <section className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-10 md:p-14">
                            ...
                            <div className="grid md:grid-cols-2 gap-10">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="h-14 flex items-center px-6 bg-slate-100/30 border border-slate-100 rounded-2xl text-slate-500 font-bold text-sm">
                                        {profile.role}
                                    </div>
                                </div>
                            </div>
